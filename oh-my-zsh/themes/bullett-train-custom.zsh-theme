...
# Prompt Hostname
prompt_stupid() {
  prompt_segment yellow blue "Hey Stupid!"
}

prompt_hostname() {
  prompt_segment cyan black $(hostname | cut -d . -f 1)
}
...
#
 ...
 RP_DATE='%{$fg[cyan]%}%{$bg[cyan]%}%{$fg[black]%} %D{%Y-%m-%d} %{$fg[white]%}%{$bg[white]%}%{$fg[black]%} %D{%H:%M}'
 RP_BATT='%{$fg[red]%}%{$bg[red]%}%{$fg[white]%} [$(~/.zsh/battery.sh)%]'
 RP_IP='%{$fg[blue]%}%{$bg[blue]%}%{$fg[white]%} $(~/.zsh/ipaddr.sh)%'
 RPROMPT=$RP_DATE' '$RP_BATT' '$RP_IP'  %{$reset_color%}%'
 TMOUT=1
 TRAPALRM() {
